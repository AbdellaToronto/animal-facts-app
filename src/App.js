...
class App extends Component {
  ...
  render() {
    return (<div className="App">
        <Toggle
          handleInputChange={e => this.setAnimalType(e.target.checked ? 'dog' : 'cat')}
          isDog={this.state.animalType === 'dog'}
        />
        {/* do not use onChange and onSubmit as props here; use the function name instead */}
        <
          MainForm value={this.state.numberOfFacts}
                   handleChange={this.handleChange}
                   handleSubmit={this.handleSubmit}
        />
        <header className="App-header">
          <Card buttonText={`Generate a ${this.state.animalType} fact`}
                facts={this.state.facts}
                handleClick={() => this.getAnimalFact(this.state.animalType, this.state.numberOfFacts)}
          />
          {/* `amount`, as set in the fetch function, is a parameter for this.state.numberOfFacts*/}
        </header>
        <section className="animalImage">
          <AnimalImage animalType={
